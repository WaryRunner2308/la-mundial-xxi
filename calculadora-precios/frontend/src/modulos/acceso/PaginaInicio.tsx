...
import { MarqueeDiagonal } from './MarqueeDiagonal';

export function PaginaInicio() {
  const { iniciarSesion } = useAuth();
  const [mostrarLogin, fijarMostrarLogin] = useState(false);
  const [credenciales, fijarCredenciales] = useState({ username: 'pumpo', password: '' });
  const [error, fijarError] = useState('');
  const [mostrarClave, fijarMostrarClave] = useState(false);
  const refClave = useRef<HTMLInputElement>(null);

  const [cargando, fijarCargando] = useState(false);

  const alEntrarInvitado = () => iniciarSesion('invitado');

  const alEnviarGerencia = async (e: React.FormEvent) => {
    e.preventDefault();
    if (cargando) return;
    fijarError('');
    fijarCargando(true);
    try {
      const exito = await iniciarSesion('gerencia', credenciales.username, credenciales.password);
      if (!exito) fijarError('Credenciales incorrectas. Verifica usuario y contraseña.');
    } catch {
      fijarError('No se pudo conectar. Revisa tu internet e intenta de nuevo.');
    } finally {
      fijarCargando(false);
    }
  };

  const alCancelarLogin = () => {
    fijarMostrarLogin(false);
    fijarError('');
    fijarCredenciales({ username: 'pumpo', password: '' });
    fijarMostrarClave(false);
  };
      ...
      <div className="max-w-2xl w-full space-y-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.025, y: -5 }}
            whileTap={{ scale: 0.975 }}
            onClick={() => fijarMostrarLogin(true)}
            className="group relative text-left overflow-hidden rounded-2xl p-6"
            style={{
        ...
        {mostrarLogin && (
          ...
          >
            ...
            >
              ...
              <form onSubmit={alEnviarGerencia} className="space-y-4" autoComplete="off" noValidate>
                <div>
                  <CampoSeguro
                    etiqueta="Usuario"
                    value={credenciales.username}
                    onChange={(value) => fijarCredenciales({ ...credenciales, username: value })}
                    alEnviar={() => refClave.current?.focus()}
                    placeholder=""
                    inputMode="text"
                    editable
                    sinAnillo
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => fijarCredenciales({ ...credenciales, password: e.target.value.replace(/[^0-9]/g, '') })}
