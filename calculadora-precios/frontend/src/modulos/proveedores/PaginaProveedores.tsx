...
export function PaginaProveedores() {
  const { proveedores, cargando, error, cargarProveedores, eliminarProveedor } = useAlmacenProveedores();
  const [proveedorEnEdicion, fijarProveedorEnEdicion] = useState<{ id: number; name: string } | null>(null);
  const [mostrarFormulario, fijarMostrarFormulario] = useState(false);
  const [confirmarEliminar, fijarConfirmarEliminar] = useState<number | null>(null);
  const [proveedorDelModal, fijarProveedorDelModal] = useState<number | null>(null);

  useEffect(() => {
    cargarProveedores().catch(() => {});
  }, [cargarProveedores]);

  const alEditar = (proveedor: { id: number; name: string }) => {
    fijarProveedorEnEdicion(proveedor);
    fijarMostrarFormulario(true);
  };

  const alAgregar = () => {
    fijarProveedorEnEdicion(null);
    fijarMostrarFormulario(true);
  };

  const alEliminar = async (id: number) => {
    try {
      await eliminarProveedor(id);
      fijarConfirmarEliminar(null);
    } catch (err) {
  ...
  const { indiceResaltado, alPresionarTecla, fijarIndiceResaltado, refContenedor } = useNavegacionTeclado({
    ...
    alSeleccionar: (proveedor) => alEditar(proveedor),
  ...
  return (
    ...
    >
                    ...
                    {proveedores.map((proveedor, indice) => {
                      ...
                      return (
                        <motion.tr
                          ...
                          onClick={() => { alEditar(proveedor); fijarIndiceResaltado(indice); }}
                          role="row"
                        ...
                        >
                          <td className="p-4 align-middle">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-sm font-semibold text-[#e6edf3]">{proveedor.name}</span>
                            </div>
                          </td>
                          <td className="p-4 align-middle">
                            <ConteoProductosProveedor
                              idProveedor={proveedor.id}
                              onOpenModal={fijarProveedorDelModal}
                            />
                          </td>
                          <td className="p-4 align-middle text-right">
                            <div className="flex items-center gap-1 justify-end">
                              <motion.button
                                ...
                                whileTap={{ scale: 0.9 }}
                                onClick={(e) => { e.stopPropagation(); alEditar(proveedor); }}
                                ...
                                title="Editar"
                                aria-label={`Editar proveedor ${proveedor.name}`}
                              >
                              ...
                              <motion.button
                                whileHover={{ scale: 1.1 }}
                                whileTap={{ scale: 0.9 }}
                                onClick={(e) => { e.stopPropagation(); fijarConfirmarEliminar(proveedor.id); }}
                                ...
                                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
                                title="Eliminar"
                                aria-label={`Eliminar proveedor ${proveedor.name}`}
                              >
      ...
      <FormularioProveedor
        ...
        alCerrar={() => fijarMostrarFormulario(false)}
        alGuardar={() => { fijarMostrarFormulario(false); cargarProveedores().catch(() => {}); }}
            ...
            >
              ...
              <div className="text-center">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => fijarConfirmarEliminar(null)}
                  ...
                  <button
                    onClick={() => alEliminar(confirmarEliminar)}
      ...
      <ModalProductosProveedor
        idProveedor={proveedorDelModal}
        alCerrar={() => fijarProveedorDelModal(null)}
      />
    </motion.div>
  );
}

function ConteoProductosProveedor({ idProveedor, onOpenModal }: { idProveedor: number; onOpenModal: (id: number) => void }) {
  const { productos } = useAlmacenProductos();
  const cantidad = productos.filter((p) => p.idProveedor === idProveedor).length;
  return (
    <button
      onClick={(e) => { e.stopPropagation(); onOpenModal(idProveedor); }}
