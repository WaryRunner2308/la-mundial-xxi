...
import type { FacturaHistorial } from '@/almacen/almacenHistorialFacturas';
import { construirResumen } from './resumenPrecios';
import { formatearMonto } from '@/utilidades/formato';
...
const CONDENSADA = '"Barlow Condensed", sans-serif';
...
const ESTILO_CIFRA = { fontFamily: MONO, fontWeight: 700, fontVariantNumeric: 'tabular-nums' } as const;
...
interface PropsResumenPreciosFactura {
  factura: FacturaHistorial | null;
  onClose: () => void;
}
 ...
 */
export function ResumenPreciosFactura({ factura, onClose }: PropsResumenPreciosFactura) {
  ...
  useEffect(() => {
    if (!abierto) return;
    const alPresionarTecla = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose(); };
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2
                      ...
                      style={{ fontFamily: CONDENSADA, fontSize: '1.35rem', letterSpacing: '0.06em' }}
                    >
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 min-w-0">
                      ...
                      <p className="text-[11px] text-[#8b949e] truncate">
                        ...
                        <span className="text-[#484f58]"> · {formatearFecha(factura.creadoEn)}</span>
                      </p>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-x-4 gap-y-1 flex-wrap mt-3">
                  ...
                  <span className="flex items-center gap-1.5 text-[10px] text-[#484f58]">
                    ...
                    <span style={{ fontFamily: MONO, color: '#8b949e' }}>
                      {factura.tasa > 0 ? `${formatearMonto(factura.tasa, 'Bs')} Bs` : 'sin tasa'}
                    </span>
                  </span>
                </div>
              </div>
            ...
            {lineas.length === 0 ? (
              <div className="px-6 py-16 text-center">
                <p
                  ...
                  style={{ fontFamily: CONDENSADA, fontSize: '1rem' }}
                >
              ...
              <div>
                {lineas.map((linea, i) => (
                  ...
                  >
                    ...
                    <p className={`${COL_BS} ${CIFRA}`} style={ESTILO_CIFRA}>
                      {linea.precioBs !== null ? formatearMonto(linea.precioBs, 'Bs') : '—'}
                    </p>
                    <p className={`${COL_USD} ${CIFRA}`} style={ESTILO_CIFRA}>
                      {linea.precioUsd !== null ? formatearMonto(linea.precioUsd, 'USD') : '—'}
                    </p>
                  </div>
                ))}
              </div>
