import type { Producto } from '@/almacen/almacenProductos';
...
export function buscarProductoExistente(
  ...
  const exacto = productos.find((p) => canonizar(p.nombre) === canonFactura);
  if (exacto) return { producto: exacto, puntaje: 1, tipo: 'exacto' };

  let mejor: CoincidenciaProducto | null = null;
  for (const producto of productos) {
    const puntaje = similitudNombre(nombreFactura, producto.nombre);
    if (puntaje >= UMBRAL && (mejor === null || puntaje > mejor.puntaje)) {
      mejor = { producto, puntaje, tipo: 'aproximado' };
    }
  }
  return mejor;
}
